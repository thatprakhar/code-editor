...
export default class WhiteBoard extends Component {
  constructor(props) {
    ...
    this.state.socket.on("drawing", data => {
      ...
      if (!isNaN(data.x0 / w) && !isNaN(data.y0)) {
        console.log("drawing coz change detected");
        this.drawLine(
          data.x0 * w,
          data.y0 * h,
          data.x1 * w,
  ...
  drawLine = (x0, y0, x1, y1, color, emit) => {
    ...
    this.setState(() => {
      if (!isNaN(x0 / w)) {
        console.log("emmitig new drawing\n");
  ...
  selectColor = color => {
    this.setState(() => {
      this.state.socket.emit("color-change", {
        color: color.hex
      });
      return {
  ...
  clearBoard = () => {
    this.state.socket.emit("clear", this.state.room);
  };
